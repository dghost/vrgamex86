...
void RemovePush(edict_t *ent)
{
	...
	if (!tpp->value && tpp_auto->value && (!cl_thirdperson->value || deathmatch->value || coop->value)
	...
	if(!client->use && (ucmd->buttons & BUTTON_USE))
	{
		...
		else
		{
			...
			if(viewing && viewing->classname)
			{
				...
				if(!Q_strcasecmp(viewing->classname,"func_monitor") && ((range <= 100) || (client->chasetoggle && range <= client->zoom + 160.00)) )
				{
...
// move viewpoint in/out
	...
	{	
		...
		viewing = LookingAt(ent,0,intersect,&range);
		if ( !(viewing && viewing->classname
			&& (Q_strcasecmp(viewing->classname,"func_monitor") || Q_strcasecmp(viewing->classname,"func_pushable"))
			&& range <= 100) )
		{
